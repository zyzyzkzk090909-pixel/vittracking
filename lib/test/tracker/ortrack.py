...
class ORTrack(BaseTracker):
    def __init__(self, params, dataset_name):
        super(ORTrack, self).__init__(params)
        ...
        self.cfg = params.cfg
        self.device = torch.device('cuda' if params.cfg.TEST.get('USE_GPU', True) and torch.cuda.is_available() else 'cpu')
    ...
    def track(self, image, info: dict = None):
        ...
        with torch.no_grad():
            ...
            out_dict = self.network.forward(
                template=self.z_dict1.tensors, search=x_dict.tensors, is_distill=self.is_distill)

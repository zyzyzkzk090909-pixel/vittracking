...
def _save_tracker_output(seq: Sequence, tracker: Tracker, output: dict):
    ...
    elif seq.dataset in ['biodrone']:
        if not os.path.exists(os.path.join(tracker.results_dir, seq.dataset, 'result')):
            os.makedirs(os.path.join(tracker.results_dir, seq.dataset, 'result'))
        if not os.path.exists(os.path.join(tracker.results_dir, seq.dataset, 'time')):
            os.makedirs(os.path.join(tracker.results_dir, seq.dataset, 'time'))
    ...
    for key, data in output.items():
        ...
        if key == 'target_bbox':
            if isinstance(data[0], (dict, OrderedDict)):
                ...
                # Single-object mode
                if seq.dataset in ['biodrone']:
                    bbox_file = os.path.join(base_results_path, f'result/{seq.name.replace("frame_", "ORTrack_")}.txt')
                    save_bb2(bbox_file, data)
                else:
        ...
        if key == 'all_scores':
            ...
            if isinstance(data[0], dict):
                ...
                if seq.dataset in ['biodrone']:
                    timings_file = os.path.join(base_results_path, f'time/{seq.name.replace("frame_", "ORTrack_")}.txt')
                else:
    ...
    def _results_exist():
        if seq.object_ids is None:
            if seq.dataset in ['trackingnet', 'got10k']:
                ...
                bbox_file = '{}/{}/{}.txt'.format(tracker.results_dir, seq.dataset, seq.name.replace('frame_', 'ORTrack_'))
            else:

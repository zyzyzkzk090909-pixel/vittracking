...
class CenterPredictor(nn.Module, ):
    def __init__(self, inplanes=64, channel=256, feat_sz=20, feat_template_sz=10, stride=16, freeze_bn=False):
        super(CenterPredictor, self).__init__()
        self.feat_sz = feat_sz
        self.feat_template_sz = feat_template_sz
        self.stride = stride
        self.img_sz = self.feat_sz * self.stride
...
def build_box_head(cfg, hidden_dim):
    ...
    if cfg.MODEL.HEAD.TYPE == "MLP":
        ...
        feat_sz = int(cfg.DATA.SEARCH.SIZE / stride)
        feat_template_sz = int(cfg.DATA.TEMPLATE.SIZE / stride)
        center_head = CenterPredictor(inplanes=in_channel, channel=out_channel,
                                      feat_sz=feat_sz, feat_template_sz=feat_template_sz,stride=stride)
        return center_head
    else:

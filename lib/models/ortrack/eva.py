...
try:
    from timm.layers import Mlp, LayerNorm, DropPath, RotaryEmbeddingCat, \
    ...
    from timm.models.layers import Mlp, LayerNorm, DropPath, \
        trunc_normal_, to_2tuple
    # Fallback to stub some missing in older timms if they exist
    apply_rot_embed_cat = None
    apply_keep_indices_nlc = None
    resample_patch_embed = None
    resample_abs_pos_embed = None
    RotaryEmbeddingCat = None
    try:
        from timm.models.layers import use_fused_attn
    except:
...
default_cfgs = generate_default_cfgs({
    ...
    'eva02_tiny_patch14_224.mim_in22k': _cfg(
        ...
        # hf_hub_id='timm/',
        url=f'file:////home/lsw/data/eva02_Ti_pt_in21k_p14.pt',
        # num_classes=0,
    ),
...
@register_model
def eva02_tiny_patch14_224_distill(pretrained=False, **kwargs) -> Eva:
    model_args = dict(
        img_size=224,
        patch_size=14,
        embed_dim=192,
